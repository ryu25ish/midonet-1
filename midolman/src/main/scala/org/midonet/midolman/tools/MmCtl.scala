...
object MmCtlResult {
    ...
    sealed abstract class MmCtlRetCode(code: Int, msg: String)
        ...
        def compare(that: MmCtlRetCode) = this.code - that.code
        def getResult: MmCtlResult = new MmCtlResult(this.code, this.msg)
        def getResult(e: Throwable): MmCtlResult = new MmCtlResult(this.code,
                                                                   this.msg)
        def getResult(msg: String): MmCtlResult = new MmCtlResult(this.code,
                                                                  this.msg)
...
trait MmCtlPortBinder {

    def bind(portId: UUID, hostId: UUID, deviceName: String): MmCtlResult
    def unbind(portId: UUID, hostId: UUID): MmCtlResult
}

object MmCtlPortBinder {

    def getBinder(injector: Injector): MmCtlPortBinder = {
        val dataClient = injector.getInstance(classOf[DataClient])
        val storage = injector.getInstance(classOf[Storage])
        val config = injector.getInstance(classOf[MidolmanConfig])

        // TODO: Change 'neutron' to 'cluster'
        if (config.neutron.enabled)
            new ZoomMmCtlPortBinder(storage)
        else
            new DataClientMmCtlPortBinder(dataClient)
    }
}

class DataClientMmCtlPortBinder(dataClient: DataClient)
    extends MmCtlPortBinder {

    import MmCtlResult._

    override def bind(portId: UUID, hostId: UUID,
                      deviceName: String): MmCtlResult = {
        try {
            dataClient.hostsAddVrnPortMapping(hostId, portId, deviceName)
        } catch {
            case e: StateAccessException =>
                return STATE_ERROR.getResult(e)
            case e: Exception =>
                return UNKNOWN_ERROR.getResult(e)
        }
        SUCCESS.getResult
    }

    override def unbind(portId: UUID, hostId: UUID): MmCtlResult = {
        try {
...
class ZoomMmCtlPortBinder(storage: Storage) extends MmCtlPortBinder {
    ...
    override def bind(portId: UUID, hostId: UUID,
        ...
        val host = storage.get(classOf[Topology.Host],
                               hostId).await().toBuilder
        val binding = host.addPortBindingsBuilder()
            .setPortId(UUIDUtil.toProto(portId))
            .setInterfaceName(deviceName)
            .build()
        host.addPortBindings(binding)
        SUCCESS.getResult
    }
    ...
    override def unbind(portId: UUID, hostId: UUID): MmCtlResult = {
        ...
        val host = storage.get(classOf[Topology.Host],
                               hostId).await().toBuilder
        val binding = host.getPortBindingsList.asScala
            .indexWhere(bdg => bdg.getPortId == UUIDUtil.toProto(hostId))
        // Unbind is idempotent
        if (binding >= 0) {
            host.removePortBindings(binding)
        }
        SUCCESS.getResult
    }
}

class MmCtl(portBinder: MmCtlPortBinder) {
    ...
    def bindPort(portId: UUID, deviceName: String): MmCtlResult = {
        try {
            ...
            case e: HostIdGenerator.PropertiesFileNotWritableException =>
                HOST_ID_NOT_FOUND.getResult(e)
        }
    }

    def bindPort(portId: UUID, hostId: UUID,
                 deviceName: String): MmCtlResult = {
        portBinder.bind(portId, hostId, deviceName)
    }
    ...
    def unbindPort(portId: UUID, hostId: UUID): MmCtlResult = {
        portBinder.unbind(portId, hostId)
    }
}
...
object MmCtl {
    ...
    def main(args: Array[String]): Unit = {
        ...
        try {
            ...
            val mmctl = new MmCtl(MmCtlPortBinder.getBinder(getInjector))
